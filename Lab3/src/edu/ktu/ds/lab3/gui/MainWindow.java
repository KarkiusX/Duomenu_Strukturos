...
import edu.ktu.ds.lab3.cpu.BenchMark;
import edu.ktu.ds.lab3.cpu.CPU;
import edu.ktu.ds.lab3.cpu.CPU_Generator;
import edu.ktu.ds.lab3.cpu.HashMapOa;
...
public class MainWindow extends BorderPane implements EventHandler<ActionEvent> {
    ...
    private ParsableMap<String, CPU> map;
    private HashMapOa<String, CPU> map1;
    private int sizeOfInitialSubSet, sizeOfGenSet, colWidth, initialCapacity;
    private float loadFactor;
    private HashType ht = HashType.DIVISION;
    private final CPU_Generator cpuGenerator = new CPU_Generator();

    enum Map_Type{
        Chain, Quadratic
    }

    Map_Type type = Map_Type.Chain;
    ...
    private void mapGeneration(String filePath) {
        ...
        if (filePath == null) {
            CPU[] cpuArrays = cpuGenerator.generateShuffleCpuAndKeys(sizeOfGenSet, sizeOfInitialSubSet);
            for (CPU c : cpuArrays) {
                map.put(
                        cpuGenerator.getCpuId(), //raktas
                        c);
            }
    ...
    public void mapEfficiency1()
    {
        ...
        BenchMark simpleBenchmark = new BenchMark(resultsLogger, semaphore);
    ...
    public void mapGeneration1()
    {
        ...
        KsGui.ounArgs(taEvents, MESSAGES.getString("mapPuts"), map1.size());

        // Atvaizdis rodomas lentelėje
        table.formTable(3, colWidth);
        String[][] modelList = map1.getModelList();;
        table.getItems().clear();
        table.setItems(FXCollections.observableArrayList(modelList));
        // Atnaujinamai maišos lentelės parametrai (geltona lentelė)
        //updateHashtableParameters(false);
        updateHashtableParameters1(false);
        // Įjungiami 2 ir 4 mygtukai
        IntStream.of(1, 3).forEach(p -> paneButtons.getButtons().get(p).setDisable(false));
    }
    private void mapPut() {
        CPU cpu = cpuGenerator.getCpu();
        map.put(
                cpuGenerator.getCpuId(), // Raktas
                cpu);
        table.formTable(map.getMaxChainSize() * 2 + 1, colWidth);
        ...
        KsGui.oun(taEvents, cpu, MESSAGES.getString("mapPut"));
    }
    private void mapPut1()
    {
        CPU cpu = cpuGenerator.getCpu();
        map1.put(
                cpuGenerator.getCpuId(), // Raktas
        ...
        String[][] modelList = map1.getModelList();
        table.setItems(FXCollections.observableArrayList(modelList));
        updateHashtableParameters1(true);
        KsGui.oun(taEvents, cpu, MESSAGES.getString("mapPut"));
    }
    ...
    private void createMap() {
        switch (cmbCollisionTypes.getSelectionModel().getSelectedIndex()) {
            case 0:
                map = new ParsableHashMap<>(String::new, CPU::new, initialCapacity, loadFactor, ht);
                type = Map_Type.Chain;
                break;
            case 1:
                map1 = new HashMapOa<>();
                type = Map_Type.Quadratic;
                break;
            default:
                IntStream.of(1, 3).forEach(p -> paneButtons.getButtons().get(p).setDisable(true));
    ...
    private void updateHashtableParameters1(boolean colorize) {
        String[] parameters = new String[]{
                String.valueOf(map1.size()),
                String.valueOf(map1.getTableCapacity()),
                String.valueOf(0),
                String.valueOf(map1.getRehashesCounter()),
                String.valueOf(0),
                String.format("%3.2f", (double) map1.size() / map1.getTableCapacity() * 100) + "%"
                // .. naujus parametrus tęsiame čia ..
        };
        for (int i = 0; i < parameters.length; i++) {
            String str = paneParam2.getTfOfTable().get(i).getText();
