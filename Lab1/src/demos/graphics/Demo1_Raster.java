...
public class Demo1_Raster extends BaseGraphics {
    ...
    public void angleLine()
    {
        ...
        double x = 0;
        for(int i=0; i<=count; i++){
            gc.strokeLine(canvasW/2-dif/2 - angle, canvasH+50, x, 0);
            gc.strokeLine(canvasW/2+dif/2 + angle, canvasH+50, x, 0);
            x += canvasW / count ;
        }
    }
    public void Circle_4()
    {
        ...
        for(int i=0; i<count; i++){
            gc.strokeOval(x-dif/2, y, canvasW-x-x, canvasH-y-y);
            gc.strokeOval(x+dif/2, y, canvasW-x-x, canvasH-y-y);
            gc.strokeOval(x-dif/2/2, y, canvasW-x-x, canvasH-y-y);
            gc.strokeOval(x+dif/2/2, y, canvasW-x-x, canvasH-y-y);
            x += dx;
            y += dy;
        }
    }
    public void RecursiveFlip(double x, double y, double w, double h)
    {
        ...
        RecursiveFlip(x,y + h/2, w / 2,  h / 2);
        RecursiveFlip(x+w/2, y + h/2, w/2, h/2);
    }
    ...
    public void createControls(){
        ...
        tfAngle = addTextField("Angle", "60", 40);
        ...
        addButton("Angle line", e -> angleLine());
        addButton("Circle", e -> Circle_4());
        addButton("Test", e ->RecursiveFlip(0, 0,canvasW, canvasH));
        addNewHBox();
    }
    @Override
