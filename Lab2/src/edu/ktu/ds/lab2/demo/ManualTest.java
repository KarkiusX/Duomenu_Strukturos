...
public class ManualTest {
    ...
    public static void executeTest() throws CloneNotSupportedException {
        ...
        Car[] carsArray = {c1, c6, c7, c8, c9};

        Ks.oun("Auto Aibė:");
        ParsableSortedSet<Car> carsSet = new ParsableBstSet<>(Car::new);
        BstSet<Car> carsSet1 = new ParsableBstSet<>(Car::new);
        carsSet.add(c8);
        carsSet.add(c5);
        carsSet.add(c6);
        carsSet.add(c1);
        ...
        carsSet.add(c4);
        carsSet1.add(c8);
        ((ParsableBstSet<Car>) carsSet).removeAll(carsSet1);
       ...
       // Ks.oun(carsSet.toVisualizedString(""));
        ...
        Ks.oun("Automobilių aibė AVL-medyje:");
                */
        /*
        ParsableSortedSet<Car> carsSetAvl = new ParsableBstSet<>(Car::new);
        for (Car c : carsArray) {
            carsSetAvl.add(c);
            Ks.ounn(c);
        }
        Ks.ounn(carsSetAvl.toVisualizedString(""));
        ((ParsableBstSet<Car>) carsSetAvl).removeAll(carsSet);
        Ks.ounn(carsSetAvl.toVisualizedString(""));
        //((ParsableBstSet<Car>) carsSetAvl).Test();
        //((ParsableBstSet<Car>) carsSetAvl).removeAll(carsSet);
       // ((ParsableBstSet<Car>) carsSetAvl).Test1();
        // Ks.ounn(carsSetAvl.toVisualizedString(""));
        //((ParsableBstSet<Car>) carsSetAvl).pollFirst();
        //Ks.ounn(carsSetAvl.toVisualizedString(""));

        //System.out.println(((ParsableAvlSet<Car>) carsSetAvl).GetLeft());
        /*
        Ks.oun("Automobilių aibė su iteratoriumi:");
        Ks.oun("");
