...
import edu.ktu.ds.lab2.cpu.CPU;
import edu.ktu.ds.lab2.cpu.CPU_Generator;
...
public class MainWindow extends BorderPane implements EventHandler<ActionEvent> {
    ...
    private static ParsableSortedSet<CPU> cpuSet;
    private CPU_Generator cpuGenerator = new CPU_Generator();
    ...
    private void treeGeneration(String filePath) throws ValidationException {
        ...
        if (filePath == null) {
           cpulist = cpuGenerator.generateShuffle(sizeOfGenSet, sizeOfInitialSubSet, shuffleCoef);
            paneParam1.getTfOfTable().get(2).setText(String.valueOf(sizeOfLeftSubSet));
        } else { // Skaitoma is failo
            cpuSet.load(filePath);
            cpulist = new CPU[cpuSet.size()];
            int i = 0;
            for (Object o : cpuSet.toArray()) {
                cpulist[i++] = (CPU) o;
            }
            // Skaitant iš failo išmaišoma standartiniu Collections.shuffle metodu.
            //Collections.shuffle(Arrays.asList(cpulist), new Random());
        }

        // Išmaišyto masyvo elementai surašomi i aibę
        cpuSet.clear();
        Arrays.stream(cpulist).forEach(cpuSet::add);

        // Išvedami rezultatai
        // Nustatoma, kad eilutės pradžioje neskaičiuotų išvedamų eilučių skaičiaus
        KsGui.setFormatStartOfLine(true);
        KsGui.oun(taOutput, cpuSet.toVisualizedString(tfDelimiter.getText()),
    ...
    private void treeAdd() throws ValidationException {
        KsGui.setFormatStartOfLine(true);
        CPU cpu = cpuGenerator.takeCpu();
        cpuSet.add(cpu);
        paneParam1.getTfOfTable().get(2).setText(String.valueOf(--sizeOfLeftSubSet));
        KsGui.oun(taOutput, cpu, MESSAGES.getString("setAdd"));
        KsGui.oun(taOutput, cpuSet.toVisualizedString(tfDelimiter.getText()));
        KsGui.setFormatStartOfLine(false);
    }
    private void treeRemove() {
        ...
        if (cpuSet.isEmpty()) {
            ...
            int nr = new Random().nextInt(cpuSet.size());
            CPU cpu = (CPU) cpuSet.toArray()[nr];
            cpuSet.remove(cpu);
            KsGui.oun(taOutput, cpu, MESSAGES.getString("setRemoval"));
            KsGui.oun(taOutput, cpuSet.toVisualizedString(tfDelimiter.getText()));
        }
        KsGui.setFormatStartOfLine(false);
    }
    private void pollFirst()
    {
        ...
        switch (type)
        {
            case Avl:
            {
                ((AvlSet<CPU>) cpuSet).pollFirst();
                break;
            }
            case Bst:
            {
                ((BstSet<CPU>) cpuSet).pollFirst();
                break;
            }
        }
        KsGui.oun(taOutput, cpuSet.toVisualizedString(tfDelimiter.getText()));
    ...
    private void RemoveALL()
    {
        ...
        int count = numb.nextInt(cpuSet.size() / 2);
        switch (type)
        {
            case Avl:
            {
                AvlSet<CPU> cpu  = new AvlSet<>();
                Add_Random(cpu, count);
                ((AvlSet<CPU>) cpuSet).removeAll(cpu);
                break;
            }
            case Bst:
            {
                BstSet<CPU> cpu = new BstSet<>();
                Add_Random(cpu, count);
                ((BstSet<CPU>) cpuSet).removeAll(cpu);
                break;
            }
        }
        KsGui.oun(taOutput, cpuSet.toVisualizedString(tfDelimiter.getText()));
    ...
    private void Add_Random(BstSet<CPU> t, int count)
    {
        for(int i =0; i < count; i++)
        {
            CPU cpu = (CPU) cpuSet.toArray()[i];
            t.add(cpu);
            System.out.println("Removing Element" + cpu);
    ...
    private void treeIteration() {
        ...
        if (cpuSet.isEmpty()) {
            ...
            KsGui.oun(taOutput, cpuSet, MESSAGES.getString("setIterator"));
        }
        KsGui.setFormatStartOfLine(false);
    }
    ...
    private void createTree() throws ValidationException {
        switch (cmbTreeType.getSelectionModel().getSelectedIndex()) {
            case 0:
                type = Tree_Type.Bst;
                cpuSet = new ParsableBstSet<>(CPU::new);
                break;
            case 1:
                type = Tree_Type.Avl;
                cpuSet = new ParsableAvlSet<>(CPU::new);
                break;
            default:
                disableButtons(true);
